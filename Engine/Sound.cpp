...
{
public:
	MFInitializer()noexcept:hr( MFStartup( MF_VERSION ) ) { }
	~MFInitializer()noexcept { if( hr == S_OK ) MFShutdown(); }
private:
	HRESULT hr = S_OK;
};
...
public:
	XAudioDll()
	{
		...
		while( true )
		{
			if( hModule = LoadLibrary( GetDllPath( type ) ); hModule != 0 ) return;
	...
	~XAudioDll()noexcept { if( hModule != 0 ) FreeLibrary( hModule ); }
...
public:
	...
	void STDMETHODCALLTYPE OnBufferEnd( void* pBufferContext )override
	{
		...
		SoundChannelController::Stop( chan );
		{
			auto lock = std::lock_guard( chan.pSound->mutex );
			auto channel_it = std::find(
				chan.pSound->activeChannelPtrs.begin(),
				chan.pSound->activeChannelPtrs.end(),
				&chan
			);
			chan.pSound->activeChannelPtrs.erase( channel_it );
		...
		chan.pSound = nullptr;
		SoundSystemController::DeactivateChannel( SoundSystemController::Get(), chan );
	}
	...
	format( std::make_unique<WAVEFORMATEX>() )
{
	...
	if( !DllGetClassObject )
	{
		...
		constexpr auto msg =
			"Getting process address of 'DllGetClassObject' function";
		throw std::system_error( err_code, msg );
	}

	// create the factory class for the XAudio2 component object
	Microsoft::WRL::ComPtr<IClassFactory> pClassFactory;
	
	if( auto hr = DllGetClassObject(
		__uuidof( XAudio2 ),
		IID_IClassFactory,
		pClassFactory.ReleaseAndGetAddressOf() ); FAILED( hr ) )
	{
		auto err_code = std::error_code( hr, std::system_category() );
		throw std::system_error( err_code, "Creating factory for XAudio2 object" );
	}

	// create the XAudio2 component object itself
	if( auto hr = pClassFactory->CreateInstance( nullptr,
		__uuidof( IXAudio2 ), &pEngine ); FAILED( hr ) )
	{
		auto err_code = std::error_code( hr, std::system_category() );
		throw std::system_error( err_code, "Creating XAudio2 object" );
	}

	// initialize the XAudio2 component object
	if( auto hr = pEngine->Initialize( 0, XAUDIO2_DEFAULT_PROCESSOR ); FAILED( hr ) )
	{
		auto err_code = std::error_code( hr, std::system_category() );
		throw std::system_error( err_code, "Initializing XAudio2 object" );
	}

	// create the mastering voice
	if( auto hr = pEngine->CreateMasteringVoice( &pMaster ); FAILED( hr ) )
	{
		auto err_code = std::error_code( hr, std::system_category() );
		throw std::system_error( err_code, "Creating mastering voice" );
	}
...
Channel::Channel( SoundSystem & sys )
	:
	xaBuffer( std::make_unique<XAUDIO2_BUFFER>() )
{
	memset( xaBuffer.get(), 0, sizeof( *xaBuffer ) );
	xaBuffer->pContext = this;
	
	if( auto hr = sys.pEngine->CreateSourceVoice( &pSource, sys.format.get(), 0u, 2.0f, &vcb ); 
		FAILED( hr ) )
	{
		auto err_code = std::error_code( hr, std::system_category() );
		throw std::system_error( err_code, "Creating source voice for channel" );
	}
}
...
Sound::Sound( const std::wstring& fileName,LoopType loopType )
{
	...
	{
		*this = SoundController::LoadNonWav( fileName,loopType,nullSample,nullSample,nullSeconds,nullSeconds );
	}
}
...
Sound::Sound( const std::wstring& fileName,unsigned int loopStart,unsigned int loopEnd )
{
	...
	{
		*this = SoundController::LoadNonWav( fileName,LoopType::ManualSample,loopStart,loopEnd,nullSeconds,nullSeconds );
	}
}
...
Sound::Sound( const std::wstring& fileName,float loopStart,float loopEnd )
{
	...
	{
		*this = SoundController::LoadNonWav( fileName,LoopType::ManualFloat,nullSample,nullSample,loopStart,loopEnd );
	}
}
	...
	try
	{
		...
		switch( loopType )
		{
			...
			case LoopType::AutoFullSound:
			{
				...
				const unsigned int nFrames = nBytes / SoundSystemController::GetFormat().nBlockAlign;
...
Sound::Sound( Sound&& donor )
{
	...
	{
		SoundChannelController::RetargetSound( *pChan, &donor,this );
	}
...
{	
	...
	if( activeChannelPtrs.size() != 0u )
	{
		...
		cvDeath.wait( lock,[this] { return activeChannelPtrs.empty(); } );
	}
