...
Surface make_plasma_ball_sprite() {
	...
	constexpr auto radius = static_cast< int >( PlasmaBall::aabb.Width() * .5f );
	constexpr auto sqrRadius = static_cast< float >( sqr( radius ) );

	for( int y = -radius; y < radius; ++y )
	{
		for( int x = -radius; x < radius; ++x )
		{
			const auto sqrDist = float( sqr( x ) + sqr( y ) );
